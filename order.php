...
if(isset($_SESSION['uid']))
{
	...
	$em = $_POST['em'];
	$ad = $_POST['ad'];
	$s= mysqli_query($conn,"select * from addcart where u_id='$uid'");
	while($r = mysqli_fetch_array($s))
	{
		$p_id = $r['p_id'];
		mysqli_query($conn,"insert into checkout(p_id, u_id,name,mobile,email,location) values('$p_id','$uid','$nm','$mo','$em','$ad')") or die("Error");
	?>
